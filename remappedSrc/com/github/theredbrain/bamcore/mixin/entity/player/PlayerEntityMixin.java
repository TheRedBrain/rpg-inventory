...
//    }
    ...
    public void dropInventory() {
        ...
        if (!this.method_48926().getGameRules().getBoolean(GameRules.KEEP_INVENTORY)) {
            ...
            if (this.method_48926().getGameRules().getBoolean(GameRulesRegistry.DESTROY_DROPPED_ITEMS_ON_DEATH)) {
                this.inventory.clear();
            } else {
                this.inventory.dropAll();
            }
            if (this.method_48926().getGameRules().getBoolean(GameRulesRegistry.CLEAR_ENDER_CHEST_ON_DEATH)) {
...
//    }
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (source.isScaledWithDifficulty()) {
            if (this.method_48926().getDifficulty() == Difficulty.PEACEFUL) {
                amount = 0.0f;
            }
            if (this.method_48926().getDifficulty() == Difficulty.EASY) {
                amount = Math.min(amount / 2.0f + 1.0f, amount);
            }
            if (this.method_48926().getDifficulty() == Difficulty.HARD) {
            ...
            if (this.blockingTime <= 20 && this.bamcore$getStamina() >= 20 && source.getAttacker() != null && source.getAttacker() instanceof LivingEntity && ((BetterAdventureMode_BasicShieldItem) shieldItemStack.getItem()).canParry()) {
                ...
                if (this.bamcore$getStamina() >= 0) {
                    ...
                    if (!isStaggered) {
                        ...
                        this.method_48926().sendEntityStatus(this, EntityStatuses.BLOCK_WITH_SHIELD);
                    } else {
                        this.method_48926().sendEntityStatus(this, EntityStatuses.BREAK_SHIELD);
                    }
                }
            } else {
    ...
    public void updateSwimming() {
        if (this.abilities.flying) {
            ...
            if (this.isSwimming()) {
                ...
                this.setSwimming(/*this.isSprinting()*/!this.hasStatusEffect(StatusEffectsRegistry.OVERBURDENED_EFFECT) && this.isSubmergedInWater() && !this.hasVehicle() && this.method_48926().getFluidState(this.getBlockPos()).isIn(FluidTags.WATER));
            }
        }
    ...
    public boolean bamcore$canConsumeItem(ItemStack itemStack) {
        ...
        if (itemStack.isFood() && list != null) {
            for (Pair<StatusEffectInstance, Float> pair : list) {
                if (method_48926().isClient || pair.getFirst() == null) continue;
