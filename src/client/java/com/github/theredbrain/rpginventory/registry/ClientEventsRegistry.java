...
public class ClientEventsRegistry {
	...
	public static void initializeClientEvents() {
		ItemTooltipCallback.EVENT.register((stack, context, type, lines) -> {
			...
			if (stack.isIn(Tags.TWO_HANDED_ITEMS) && clientConfig.itemTooltipSection.show_item_tooltip_two_handed_items.get()) {
				int index = clientConfig.itemTooltipSection.item_tooltip_two_handed_items_index.get();
				if (index < 0) {
	...
	private static List<Text> getLoadOutItemsTooltipLines(boolean showLoadOutDescription) {
		List<Text> lines = new ArrayList<>();
		lines.add(Text.translatable("item.additional_tooltip.load_out_item.line_1"));
		if (showLoadOutDescription) {
			lines.add(Text.translatable("item.additional_tooltip.load_out_item.line_2"));
		}
		return lines;
	}

	private static List<Text> getPlayerBoundItemsTooltipLines(ItemStack stack) {
		List<Text> lines = new ArrayList<>();
	...
	private static List<Text> getPlayerCraftedItemsTooltipLines(ItemStack stack) {
		List<Text> lines = new ArrayList<>();
	...
	private static List<Text> getAdvancementLockedItemsTooltipLines(ItemStack stack) {
		List<Text> lines = new ArrayList<>();
	...
	private static List<Text> getEquipmentSlotTooltipLines(ItemStack stack) {
		List<Text> lines = new ArrayList<>();
