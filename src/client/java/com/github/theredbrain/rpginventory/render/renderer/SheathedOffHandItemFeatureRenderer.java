...
public class SheathedOffHandItemFeatureRenderer<T extends LivingEntity> extends ItemInHandLayer<T, PlayerModel<T>> {

	private final ItemInHandRenderer heldItemRenderer;

	public SheathedOffHandItemFeatureRenderer(RenderLayerParent<T, PlayerModel<T>> context, ItemInHandRenderer heldItemRenderer) {
	...
	public void render(PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
		...
		if (livingEntity instanceof RendersSheathedWeapons renderEquippedTrinkets) {
			...
			if (!offHandStack.isEmpty() && !offHandStack.is(Tags.NOT_SHOWN_WHEN_IN_SHEATHED_OFFHAND)) {
				...
				String itemId = BuiltInRegistries.ITEM.getKey(offHandItem).toString();
				...
				heldItemRenderer.renderItem(livingEntity, offHandStack, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, false, matrixStack, vertexConsumerProvider, i);
				matrixStack.popPose();
			}
		}
	}
