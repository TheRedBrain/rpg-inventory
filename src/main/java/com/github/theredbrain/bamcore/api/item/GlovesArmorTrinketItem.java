...
public class GlovesArmorTrinketItem extends TrinketItem {
    ...
    public GlovesArmorTrinketItem(double armor, double armorToughness, double weight, Settings settings) {
        super(settings);
        this.armor = armor;
    ...
    public String getTranslationKey(ItemStack stack) {
        return ((GlovesArmorTrinketItem)stack.getItem()).isProtecting(stack) ? this.getTranslationKey() : this.getOrCreateTranslationKeyBroken();
    }

    @Override
    public Multimap<EntityAttribute, EntityAttributeModifier> getModifiers(ItemStack stack, SlotReference slot, LivingEntity entity, UUID uuid) {
        Multimap<EntityAttribute, EntityAttributeModifier> map = super.getModifiers(stack, slot, entity, uuid);
        if (slot.inventory().getSlotType().getGroup().equals("gloves")) {
            if (this.isProtecting(stack)) {
                map.put(EntityAttributes.GENERIC_ARMOR,
                        new EntityAttributeModifier(UUID.fromString(BetterAdventureModCoreAttributeModifierUUIDs.GLOVE_SLOT),
