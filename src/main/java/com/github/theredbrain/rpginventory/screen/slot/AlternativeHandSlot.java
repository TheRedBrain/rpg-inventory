...
public class AlternativeHandSlot extends Slot {
	...
	public AlternativeHandSlot(Container inventory, Player playerEntity, EquipmentSlot equipmentSlot, int index, int x, int y, @Nullable Identifier backgroundSprite, List<Component> tooltip) {
		this(inventory, playerEntity, equipmentSlot, index, x, y, backgroundSprite, tooltip, false);
	}

	public AlternativeHandSlot(Container inventory, Player playerEntity, EquipmentSlot equipmentSlot, int index, int x, int y, @Nullable Identifier backgroundSprite, List<Component> tooltip, boolean allowsLoadoutItemRemoval) {
	...
	public boolean mayPickup(Player playerEntity) {
		...
		) && (!itemStack.has(RPGInventory.LOAD_OUT_ITEM) || this.allowsLoadoutItemRemoval) && (itemStack.has(RPGInventory.IGNORES_EQUIPMENT_CHANGE_RESTRICTIONS) || owner.hasEffect(RPGInventory.CIVILISATION) || isCreative || (RPGInventory.SERVER_CONFIG.allow_equipment_changes.get() && !owner.hasEffect(RPGInventory.WILDERNESS)));
	}
